...
10. **Match a document's length to what the task needs** — cover the substance, don't pad with
    filler sections, redundant summaries, or boilerplate. (The user's output style carries this
    for the main conversation; an output style does not reach a subagent, so it is restated here.)

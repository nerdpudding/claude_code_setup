...
hierarchy (`AI_INSTRUCTIONS.md`) as the map; don't spawn agents for this.

**Project boundary (`[user-specified]` 2026-07-19): clusters contain ONLY the invoking
project's own files.** Sibling/other projects are never swept — not read into clusters, and
their files are never edited in the fix phase — unless the user explicitly asked for that
project in this run. A cross-project inconsistency you happen to see is reported as a watch
item for the user to take to that project; it is never fixed from here.

Per cluster set:
- `name` and `paths` (real file paths);
- `judgment: true` marks clusters needing real judgment (design docs, cross-referenced status);
  every reader runs on sonnet either way;
...
project's agent definitions restate — and always the project boundary from step 1 ("this
project's files only; anything cross-project is a watch item, never a finding to fix"). This
string is appended to every agent prompt.
...
```
Workflow({ name: 'doc-sweep-fleet', args: { clusters, guardrails } })
```

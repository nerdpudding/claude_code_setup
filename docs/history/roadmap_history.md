# Roadmap history — claude-code-setup
...
## Sprint 13: Opus 5.5 as the main model — make sure it is the one running, then align to it (Done — 2026-09-25)
...
Closed the same day; no plan was needed.

1. The Opus 5 session was the switch-over: Claude Code 2.1.280 made Opus 5.5 the default Opus
   model, and `opus[1m]` now resolves to it. The pin stays an alias.
2. Anthropic's Opus 5.5 guide is in `docs/prompting_guides/prompting-claude-opus-5-5.md`. It says
   Opus 5 prompts carry over unchanged. New in 5.5: default effort `medium`, and it can end a
   long turn with a progress report instead of continuing. Anthropic's "never stop to check in"
   paragraph is for unattended agents and is not adopted: it conflicts with plan approval.
3. The one change that mattered: a top-level `effortLevel` no longer reaches Opus 5.5, so the
   main session had silently run at `medium`. Now pinned as
   `modelSettings."claude-opus-5-5".effortLevel: "high"`; `docs/model_alignment/opus_5_alignment.md`
   and `README.md` corrected (commit `b1b5cdf`).
4. No field reports yet on Opus 5.5 as a Claude Code main agent; the model shipped 2026-09-22.
5. Global `CLAUDE.md` and the Personal Voice output style unchanged.
6. The test is ordinary work on 5.5 — see the backlog.

## Sprint 10: Telemetry, feature-gating and what "off" actually costs (Done — 2026-09-25)
...
installed binary — see the session in another project that raised it):**
...
Re-checked against the data-usage docs, the installed 2.1.282 binary and the GitHub issues.

- Telemetry stays off. The coupling is real and unchanged: `DISABLE_TELEMETRY` also switches off
  feature-flag evaluation. In 2.1.282 the only feature that states it needs flags is **Remote
  Control**. No flag gate on the 1M model was found; `opus` resolves to Opus 5.5 with telemetry off.
- `DISABLE_FEEDBACK_COMMAND=1` added to `global_config/settings.json` and the live settings
  (commit `66d4f80`). `CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY` is not needed: telemetry-off already
  disables the survey.
- Issue status: #29580 (Remote Control) open since 2026-02-28, 15 user comments, no response from
  Anthropic; #58383 fixed for Agent View only; #34178 and #62382 closed "not planned"; #38450 and
  #45918 closed as duplicates.
- The user asked Anthropic on #29580 whether they plan to separate flag fetching from telemetry:
  https://github.com/anthropics/claude-code/issues/29580#issuecomment-5836700350
- Not done, by decision: the alternatives question and the Monitor-tool question. The
  data-privacy toggle on claude.ai is the user's own check.
...
the v2.7 entry in `README.md` for the change list. Plan, with the independent review and the
deviations as built: `archive/2026-09-19_PLAN_single_home.md` (local only — `archive/` is not
published).

- [x] A snapshot of the live config that carried the home path removed from the repo and from its
      git history (`git filter-branch`, mirror backup first, fresh-clone check after the push)
- [x] `global_config/CLAUDE.md`: the tree-and-pointer rule as single home; compaction line; three
      passages cut to one statement each
- [x] Output style: mannered-prose paragraph, progress-update line, "ask only what the user can judge"
- [x] `/realign`: copied project facts (check 3), rules in auto-memory (check 7), tree and pointer
      findings (check 16), one finding per turn without priority codes, name-based pointers
- [x] `/project-setup`: stale `haiku` line out, tier pointer, Phase 0 on `./install.sh diff`
- [x] `/feature-close`, `/custom_plan`, `/doc-sweep`, `milestone-review.js`: tier pointers, wider
      history diet, conditional live end-to-end, priorities in words
...
Token totals: independent plan review on `opus` **171,993** subagent tokens (33 tool calls,
~8 min); build agent on `opus` **210,469** (92 tool calls, ~14 min); closing doc-keeper pass on
`sonnet` **216,649** (44 tool calls, ~10 min). Main-session total is not visible from inside the session: read
`/usage`.

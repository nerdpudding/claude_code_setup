...
- There used to be a separate step 3 that re-read the session to check for omissions. Removed:
  Opus 5 does that pass on its own, and instructing it caused a second full re-read for nothing.
  The bar itself has not moved — it lives in the hard rules and the acceptance test above.

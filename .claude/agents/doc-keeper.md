...
4. `concepts/concept.md` — detailed concept and design
5. Check `global_config/` — the template files this project distributes
...
1. **Audit documentation state** — compare filesystem against documented hierarchies in AI_INSTRUCTIONS.md
2. **Detect stale content** — cross-reference data across documents for mismatches (e.g., resources table in concept.md vs actual files in global_config/)
...
6. **Verify template files** — ensure the files in global_config/ match what's documented
7. **Check agents table** — verify the agents table in AI_INSTRUCTIONS.md matches actual `.claude/agents/` contents
...
9. Neutral tone — avoid "we", "our", "us" in documentation. Keep language general and impersonal unless explicitly asked otherwise

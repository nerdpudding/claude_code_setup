...
If `sessions/SESSION_CARRYOVER.md` existed, move it to
`archive/<YYYY-MM-DD_HHMM>_SESSION_CARRYOVER.md` — date AND time (run `date +%F_%H%M`, never
guess), so the name is UNIQUE even when several sessions hand over on the same day and the
move never collides with an existing archived carryover (create `archive/` if missing). The
archive exists so it is never mistaken for a still-active handover. If there was no
carryover, skip this.

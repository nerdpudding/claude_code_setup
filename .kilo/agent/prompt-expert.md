---
# GENERATED by kilo-sync from .claude/agents/prompt-expert.md - do not edit; edit the source and re-run.
...
**Judge it for every model that will read it.** A rule in the global `AGENTS.md`, in the output
style or in a skill is read by the session model and by every subagent, and those run different
models — "Model tiers for subagents" in the global `AGENTS.md` says which model does what kind of

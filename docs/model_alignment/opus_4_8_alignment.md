...
6. **Lean always-loaded surface.** `CLAUDE.md` loads in full every session and longer files reduce
   adherence; Anthropic's target is **under 200 lines**. Push detail into focused sub-docs read on
   demand, and give each agent a targeted read of the one doc it needs rather than a blanket
   "read the whole 29KB file".

   > **Two corrections, 2026-07-30**, made against the official docs after this document had been
   > distributing them for two months:
   > - **`@path` imports do NOT reduce context** — imported files load at launch too. Only three
   >   things shrink the loaded surface: deleting content, moving procedure into a **skill** (loads
   >   on invocation), or a **path-scoped `.claude/rules/` file** (loads on matching files).
   > - **Subagents DO receive the CLAUDE.md hierarchy** (all but the built-in `Explore` and `Plan`).
   >   The original claim that they don't was wrong. They do NOT get the output style, the main
   >   conversation's auto memory, or its history — so restating CLAUDE.md rules in an agent
   >   definition is duplication, while tone and memory facts an agent needs must be in its prompt.

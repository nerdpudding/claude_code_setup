export const meta = {
  ...
  description: 'Whole-codebase review at milestones: per-dimension finders, adversarial refuters, one synthesis into the review doc (pass args.reviewFile, e.g. docs/Review_<date>.md)',
  phases: [
    ...
    { title: 'Synthesize', detail: 'one synthesis writes the prioritized findings into the review doc' },
  ],
}
...
// it do not change — the findings are a REVIEW RECORD, not a plan (user ruling 2026-07-19):
// triage lands on the roadmap, and a later custom plan decides the fix-sprint scope.
//
...
//   1. Pass the review doc the findings should land in — scripts cannot compute dates, so
//      the INVOKER dates the filename (convention: docs/Review_<day>_<Month>.md).
//   2. Invoke in a turn that states a token target ("+500k" — hard ceiling here):
//      Workflow({ name: 'milestone-review', args: { reviewFile: 'docs/Review_<date>.md',
//                 guardrails: '<the project's privacy/content rules>' } })
//   Bare invocation no longer throws: it falls back to docs/REVIEW_milestone.md (undated)
//   and logs the recommendation; rename afterward if the fallback was used.
//
...
// findings section into the review doc ("findings land in files, not in chat"). A failed
...
//
// args = {
//   reviewFile?: string,         // the review doc the findings section goes into (dated by
//                                // the invoker, e.g. docs/Review_19_July.md); alias:
//                                // planFile (pre-2026-07-19 invocations); default:
//                                // docs/REVIEW_milestone.md (undated fallback, logged)
...
const a = args || {}
const reviewFile =
  (typeof a.reviewFile === 'string' && a.reviewFile) ||
  (typeof a.planFile === 'string' && a.planFile) || // pre-2026-07-19 alias
  'docs/REVIEW_milestone.md'
if (reviewFile === 'docs/REVIEW_milestone.md') {
  log(
    'milestone-review: no reviewFile passed — writing to the undated fallback docs/REVIEW_milestone.md; prefer a dated args.reviewFile (e.g. docs/Review_<day>_<Month>.md) and rename afterward if needed',
  )
}
const dims = Array.isArray(a.dimensions) && a.dimensions.length ? a.dimensions : DEFAULT_DIMENSIONS
...
// Always-on: an output style never reaches a subagent, so the user's document-length rule
// has to be restated here or every agent-authored file comes back padded.
const LENGTH_RULE =
  '\nMatch the length of anything you write to what the task needs: cover the substance, do not pad with filler sections, redundant summaries, or boilerplate.'

const guardrails =
  LENGTH_RULE +
  (a.guardrails ? `\nProject guard rails (apply strictly): ${a.guardrails}` : '')
...
if (survivors.length === 0) {
  log('milestone-review: no findings survived the refuters — nothing written to the review doc')
...
const summary = await agent(
    ...
    guardrails,
  {
    ...
    model: a.synthesisModel || 'opus',
  },
)

if (!summary) {
  log('milestone-review: synthesis returned nothing — findings were NOT written; re-run or resume before trusting the review doc')
